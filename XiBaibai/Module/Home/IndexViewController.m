...
//- (void)initButtomScroll{
//    
//    /**
//     * @brief 底部背景视图
//     **/
//    _bottomBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 100, self.view.frame.size.width, 100)];
//    _bottomBackgroundView.backgroundColor = [UIColor whiteColor];
//    [self.view addSubview:_bottomBackgroundView];
//    _selectedBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 130, 100)];
//    _selectedBackgroundView.center = CGPointMake(_bottomBackgroundView.frame.size.width * 0.5f, _bottomBackgroundView.frame.size.height * 0.5f);
//    _selectedBackgroundView.backgroundColor = [UIColor orangeColor];
//    [_bottomBackgroundView addSubview:_selectedBackgroundView];
//    _contentScl = [[UIScrollView alloc] initWithFrame:_bottomBackgroundView.bounds];
//    _contentScl.backgroundColor = [UIColor clearColor];
//    _contentScl.contentInset = UIEdgeInsetsMake(0, _selectedBackgroundView.frame.origin.x, 0, _selectedBackgroundView.frame.origin.x);
//    _contentScl.scrollEnabled = NO;
//    [_bottomBackgroundView addSubview:_contentScl];
//   
//   
//    
...
//                DLog(@"%@      %@",endReminderTime,startReminderTime);
//            }
//        }
//    } failBlock:^(NSError *error) {
...
//    
//    //请求产品
//    [NetworkHelper postWithAPI:API_Pro_select parameter:nil successBlock:^(id response) {
//        NSLog(@"dics%@",response);
//        arrProducts = [response objectForKey:@"result"];
//        DLog(@"arrProducts%@",arrProducts);
//        for (int i = 0; i < [arrProducts count]; i++) {
//            UIView *item = [[UIView alloc] initWithFrame:CGRectMake(i * 130, 0, 130, 100)];
//            [item addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapGesture:)]];
//            item.tag = i;
//            item.backgroundColor = [UIColor clearColor];
//            
//            UIView *viewProduct = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 130, 100)];
//            viewProduct.userInteractionEnabled = YES;
//            viewProduct.tag = [[arrProducts[i] objectForKey:@"id"] integerValue];
//            viewProduct.userInteractionEnabled = YES;
//            UILabel *labName = [[UILabel alloc] initWithFrame:CGRectMake(10, 15, 55, 20)];
//            labName.textColor = [UIColor whiteColor];
//            labName.font = [UIFont boldSystemFontOfSize:17.0f];
//            labName.text = [arrProducts[i] objectForKey:@"p_name"];
//            [viewProduct addSubview:labName];
//            
//            UILabel *labprice = [[UILabel alloc] initWithFrame:CGRectMake(70, 15, 60, 20)];
//            labprice.textColor = [UIColor whiteColor];
//            NSArray *arrprice = [[arrProducts[i] objectForKey:@"p_price"] componentsSeparatedByString:@"."];
//            labprice.text=[NSString stringWithFormat:@"￥%@",arrprice[0]];
//            [viewProduct addSubview:labprice];
//            if (i == arrProducts.count - 1) {
//                labprice.text = @"";
//            }
//            
//            UILabel *labInfo=[[UILabel alloc] initWithFrame:CGRectMake(10, 40, 110, 80)];
//            labInfo.numberOfLines=0;
//            labInfo.textColor = [UIColor whiteColor];
//            //labInfo.textColor=kUIColorFromRGB(0xfee6e0);
//            labInfo.font=[UIFont fontWithName:nil size:15.0];
//            labInfo.text=[arrProducts[i] objectForKey:@"p_info"];
//            
//            //调整行间距
//            NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:[arrProducts[i] objectForKey:@"p_info"]];
//            NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
//            
////            [paragraphStyle setLineSpacing:5.0];//调整行间距
//            
//            [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [[arrProducts[i] objectForKey:@"p_info"] length])];
//            labInfo.attributedText = attributedString;
//            [viewProduct addSubview:labInfo];
//            [labInfo sizeToFit];
//            
//            if(i==0){
//                p_id=[NSString stringWithFormat:@"%ld",viewProduct.tag];
//                
//                total_price = [NSString stringWithFormat:@"%@",labprice.text];
//                
//                [item addSubview:viewProduct];
...
//            }
//            
//            [_contentScl addSubview:item];
//        }
//        _contentScl.contentSize = CGSizeMake([arrProducts count]*130, 0);
//
//    } failBlock:^(NSError *error) {
//        [SVProgressHUD showErrorWithStatus:@"网络错误"];
//    }];
//    
//    UISwipeGestureRecognizer *leftSwipeGes = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeGesture:)];
//    leftSwipeGes.direction = UISwipeGestureRecognizerDirectionLeft;
//    [_bottomBackgroundView addGestureRecognizer:leftSwipeGes];
//    
//    UISwipeGestureRecognizer *rightSwipeGes = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeGesture:)];
//    rightSwipeGes.direction = UISwipeGestureRecognizerDirectionRight;
//    [_bottomBackgroundView addGestureRecognizer:rightSwipeGes];
//
//}

//- (void)handleTapGesture:(UITapGestureRecognizer *)sender {
//    
//    if (self.selectedIndex == arrProducts.count - 1) {
//        if (sender.view.tag < arrProducts.count - 1) {
////            self.selectedIndex = (int)sender.view.tag;
////         
////            for (UIView *temp1 in _contentScl.subviews) {
////                for (UIView *temp2 in temp1.subviews) {
////                    for (UIView *temp3 in temp2.subviews) {
////                        if ([temp3 isKindOfClass:[UILabel class]]) {
////                            UILabel *lbl = (UILabel *)temp3;
////                            lbl.textColor = [UIColor blackColor];
////                        }
////                    }
////                }
////                if (temp1 == _contentScl.subviews[self.selectedIndex]) {
////                    for (UIView *temp2 in temp1.subviews) {
////                        for (UIView *temp3 in temp2.subviews) {
////                            if ([temp3 isKindOfClass:[UILabel class]]) {
////                                UILabel *lbl = (UILabel *)temp3;
...
////            [UIView animateWithDuration:0.25 animations:^{
////                self.contentScl.contentOffset = CGPointMake(sender.view.tag * 130 - self.contentScl.contentInset.left, 0);
////            }];
//        } else {
//            if (IsLogin)
//                [self.navigationController pushViewController:[[DIYViewController alloc] init] animated:YES];
//            else
//                GoToLogin(self);
//            return;
//        }
...
//            return;
//        }
//        if (ceil(_contentScl.contentOffset.x + _contentScl.contentInset.left) < _contentScl.contentSize.width - 130) {
//            [UIView animateWithDuration:0.25f animations:^{
//                _contentScl.contentOffset = CGPointMake(_contentScl.contentOffset.x + 130, 0);
//            }];
//            self.selectedIndex++;
//        }
//    } else if (sender.direction == UISwipeGestureRecognizerDirectionRight) {
//        if (ceil(_contentScl.contentOffset.x + _contentScl.contentInset.left) >= 130) {
//            [UIView animateWithDuration:0.25f animations:^{
//                _contentScl.contentOffset = CGPointMake(_contentScl.contentOffset.x - 130, 0);
//            }];
//            self.selectedIndex--;
...
//        NSDictionary *dic = arrProducts[selectedIndex];
//        p_id = dic[@"id"];
//    }
////    lab.text = [NSString stringWithFormat:@"%d",_selectedIndex];
////    [_selectedBackgroundView addSubview:lab];
//    NSLog(@"_____%d",_selectedIndex);
//    // to do....
//}
//更新用户头像信息
...
//        
//        
//        AddPlanOrderViewController *addplanController=[[AddPlanOrderViewController alloc] init];
//        addplanController.c_id = [UserObj shareInstance].c_id;
//        addplanController.p_id=p_id;
//        addplanController.location=address_chose;
//        addplanController.location_lg=location_lg;
//        addplanController.location_lt=location_lt;
//        addplanController.total_price=total_price;
//        
//        
//        
//        
////        NewOrderViewController *newOrder = [[NewOrderViewController alloc] init];
////        newOrder.isReservation = YES;
////        newOrder.navtitle = @"预约上门";
//        [self.navigationController pushViewController:addplanController animated:YES];
//
//    }
//    else
//    {
